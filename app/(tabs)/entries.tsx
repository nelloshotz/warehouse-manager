...
import { ArrowDownToLine, Search } from "lucide-react-native";
...
export default function EntriesScreen() {
  ...
  useEffect(() => {
    if (selectedMonth) {
      const loadMonthData = async () => {
        const rows = await getDocumentRowsByMonth(selectedMonth, 'entry');
  ...
  const totalCost = entrySummaries.reduce((sum, entry) => sum + entry.costo_ingresso, 0);
  const totalPallets100x120 = entrySummaries.reduce((sum, entry) => sum + entry.numero_bancali_ingresso_100x120, 0);
  const totalPallets80x120 = entrySummaries.reduce((sum, entry) => sum + entry.numero_bancali_ingresso_80x120, 0);
  const totalEquivalence = entrySummaries.reduce((sum, entry) => sum + entry.equivalenza_100x120, 0);
  const totalPallets = entrySummaries.reduce((sum, entry) => sum + entry.totale_bancali, 0);
  
  const handleRowPress = (item: any) => {
    setSelectedMonth(item.mese_ingresso);
  ...
  const columns = [
    ...
    {
      key: "numero_bancali_ingresso_100x120",
    ...
    {
      key: "numero_bancali_ingresso_80x120",
    ...
    <AppLayout>
      ...
      >
        ...
        <View style={styles.tableContainer}>
          <Text style={styles.tableTitle}>Ingressi Mensili</Text>
          <DataTable 
            data={entrySummaries} 
            columns={columns} 
            emptyMessage="Nessun dato di ingresso disponibile. Carica un file per vedere gli ingressi."
