...
export function buildFinishedProductsReportFromCsv(
  ...
  for (let i = 1; i < lines.length; i += 1) {
    ...
    const descrizioneOriginal = sanitizeProductName(row[descrizioneColumnIndex] || "");
    const descrizione = normalizeName(descrizioneOriginal);
    ...
    const currentTotal = productTotals.get(descrizioneOriginal) || 0;
    productTotals.set(descrizioneOriginal, currentTotal + giacenzaRiga);
  }

  const results: FinishedProductRow[] = Array.from(productTotals.entries()).map(
    ([nome, giacenza]) => ({
      nome_prodotto: nome,
      giacenza_bancali: Number(giacenza.toFixed(3)),
    })
  );

  return results.sort((a, b) => {
    const letterA = getGroupLetter(a.nome_prodotto);
    const letterB = getGroupLetter(b.nome_prodotto);
    ...
    return a.nome_prodotto.localeCompare(b.nome_prodotto, "it", {
      sensitivity: "base",
    });
  });
}
